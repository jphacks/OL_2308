...
def prepare(q1):
    prepare_dict = prepare_ladi_vton()
    q1.put(prepare_dict)
    print("preparation_finished")
...
def generate_semantic_img(ori_img):
    ...
    img_seg=np.where(img_seg==0,245,img_seg)
...
def run_vton(prepare_dict, category):
    ...
    os.chdir("./ladi_vton")
    generate_img(prepare_dict, category)
    os.chdir("../")

    # Add Background or Not
    l=glob.glob("./output/*.jpg")
    enttime = time.time()-starttime
    print(enttime)
