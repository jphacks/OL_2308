...
from transformers import (AutoConfig, CLIPTextModel,
...
def inversion_adapter(dataset: Literal['dresscode', 'vitonhd']):
    ...
    #inversion_adapter.load_state_dict(torch.load(f"./src/models/inversion_adapter_{dataset}.pth"))
...
def extended_unet(dataset: Literal['dresscode', 'vitonhd']):
    ...
    #unet.load_state_dict(torch.load(f"./src/models/unet_{dataset}.pth"))
    return unet
...
def emasc(dataset: Literal['dresscode', 'vitonhd']):
    ...
    #emasc.load_state_dict(torch.load(f"./src/models/emasc_{dataset}.pth"))
    return emasc
...
def warping_module(dataset: Literal['dresscode', 'vitonhd']):
    ...
    #tps.load_state_dict(torch.load(f"./src/models/warping_{dataset}.pth")['tps'])
    #refinement.load_state_dict(torch.load(f"./src/models/warping_{dataset}.pth")['refinement'])
