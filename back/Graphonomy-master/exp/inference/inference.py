...
def inference(net, img_path='', output_path='./', output_name='f', use_gpu=True):
    ...
    #parsing_im = parsing_im.resize((768, 1024))
    parsing_im.save(output_path+'/{}.png'.format(output_name))
    cv2.imwrite(output_path+'/{}_gray.png'.format(output_name), results[0, :, :])
    #resized_img = cv2.imread(output_path+'/{}_gray.png'.format(output_name), cv2.IMREAD_GRAYSCALE)
    #resized_img = cv2.resize(resized_img, (768, 1024))
    #cv2.imwrite(output_path+'/{}_gray.png'.format(output_name), resized_img)
