...
from src.utils.posemap import (get_coco_body18_mapping,
                               get_coco_body25_mapping, kpoint_to_heatmap)

from .get_human_agnostic import get_img_agnostic, get_upper_inpainting_mask
...
class VitonHDDataset(data.Dataset):
    def __init__(self,
        ...
        possible_outputs = ['c_name', 'im_name', 'cloth', 'image', 'im_cloth', 'shape', 'im_head', 'im_pose',
                            ...
                            'category', 'warped_cloth', 'clip_cloth_features', 'mask_hair', 'image_row']
    ...
    def __getitem__(self, index):
        ...
        if "cloth" in self.outputlist:  # In-shop clothing image
            ...
            cloth_mask = Image.open(os.path.join(dataroot, self.phase, 'cloth-mask', c_name)).convert("1")
            cloth_canvas = Image.new("RGB", (cloth.width, cloth.height), (255, 255, 255))
            cloth_canvas.paste(cloth, None, cloth_mask)
            #cloth_canvas.show()
            cloth = cloth_canvas.resize((self.width, self.height))
            cloth = self.transform(cloth)  # [-1,1]

        if "image" in self.outputlist or "im_head" in self.outputlist or "im_cloth" in self.outputlist:
            # Person image
            image_row = Image.open(os.path.join(dataroot, self.phase, 'image', im_name))
            image_row = image_row.resize((self.width, self.height))
            image = self.transform(image_row)  # [-1,1]
        ...
        if "im_pose" in self.outputlist or "parser_mask" in self.outputlist or "im_mask" in self.outputlist or "parse_mask_total" in self.outputlist or "parse_array" in self.outputlist or "pose_map" in self.outputlist or "parse_array" in self.outputlist or "shape" in self.outputlist or "im_head" in self.outputlist:
            ...
            mask_hair = (parse_array == 2).astype(np.float32)
            ...
            if category == "upper_body":
                ...
                parse_mask1 = (parse_array == 5).astype(np.float32) + \
                            (parse_array == 6).astype(np.float32) + \
                            (parse_array == 7).astype(np.float32) + \
                ...
                parser_mask_fixed = (parse_array == 2).astype(np.float32) + \
                                    (parse_array == 18).astype(np.float32) + \
                                    (parse_array == 19).astype(np.float32) + \
                ...
                parse_mask = (parse_array == 9).astype(np.float32) + \
                            (parse_array == 16).astype(np.float32) + \
                            (parse_array == 17).astype(np.float32)

                parser_mask_fixed += (parse_array == 5).astype(np.float32) + (parse_array == 14).astype(np.float32) + (parse_array == 15).astype(np.float32)
            ...
            with open(os.path.join(dataroot, self.phase, 'openpose_json', pose_name), 'r') as f:
                ...
                pose_data[:, 0] = pose_data[:, 0] * (self.width / 384)  #########################
                pose_data[:, 1] = pose_data[:, 1] * (self.height / 512) #########################

            pose_mapping = get_coco_body18_mapping()
            ...
            with open(os.path.join(dataroot, self.phase, 'openpose_json', pose_name), 'r') as f:
                ...
                # rescale keypoints on the base of height and width
                data[:, 0] = data[:, 0] * (self.width / 384)  #########################
                data[:, 1] = data[:, 1] * (self.height / 512) #########################
                ...
                ARM_LINE_WIDTH = int(45 / 512 * self.height)  #########################
                ...
                if category == "upper_body":
                    ...
                    parse_mask += im_arms
                    #mask = Image.new("1", (image_row.size[0], image_row.size[1]))  # 追加
                    #parse_mask = get_upper_inpainting_mask(mask, im_parse, data)  # 追加
                    parse_mask1 = Image.fromarray(parse_mask1*255)
                    parse_mask1 = parse_mask1.convert("1")
                    parse_mask = get_upper_inpainting_mask(parse_mask1, im_parse, data)  # 追加
            ...
            # mask torso
            """parse_mask = Image.fromarray(parse_mask*255)
            parse_mask = parse_mask.convert("1")
            torso_draw = ImageDraw.Draw(parse_mask)
            length_a = np.linalg.norm(data[5] - data[2])
            w_torso = int(length_a / 16) + 1
            for i in [8, 11]:
                pointx, pointy = data[i]
                torso_draw.ellipse((pointx-w_torso*3, pointy-w_torso*6, pointx+w_torso*3, pointy+w_torso*6), 'white', 'white')
            torso_draw.line([tuple(data[i]) for i in [2, 8]], 'white', width=w_torso*6)
            torso_draw.line([tuple(data[i]) for i in [5, 11]], 'white', width=w_torso*6)
            torso_draw.line([tuple(data[i]) for i in [8, 11]], 'white', width=w_torso*12)
            torso_draw.polygon([tuple(data[i]) for i in [2, 5, 11, 8]], 'white', 'white')
            parse_mask.show()
            parse_mask = np.uint8(parse_mask)"""
            ...
            """cv2.imshow('img',np.array(parse_mask, dtype=np.uint8)*255)
            cv2.waitKey()
            cv2.destroyAllWindows()"""
            parse_mask_total = np.logical_or(parse_mask, parser_mask_fixed)
            """cv2.imshow('img',np.array(parse_mask_total, dtype=np.uint8)*255)
            cv2.waitKey()
            cv2.destroyAllWindows()"""
            im_mask = image * parse_mask_total
            inpaint_mask = 1 - parse_mask_total
            """cv2.imshow('img',np.array(inpaint_mask, dtype=np.uint8)*255)
            cv2.waitKey()
            cv2.destroyAllWindows()"""
        ...
        for k in self.outputlist:
            ...
            print(k)
            print(vars()[k])
            if type(vars()[k]) == type(image):
                print((vars()[k]).dtype)
                print((vars()[k]).shape)
                print("max_value", torch.max(vars()[k]))"""
            result[k] = vars()[k]
